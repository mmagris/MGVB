classdef MGVBb < VBayesLab
    ...
    methods
        function obj = MGVBb(mdl,data,varargin)
        ...
        function Post = fit(obj,data)
            ...
            if save_params
                ...
                for b = 1:n_blks
                    [L{b,1},Sig_inv{b,1}] = mk_L_Sig(Sig{b});
                end
            ...
            while ~stop
                ...
                Sig{b,1}     = obj.retraction_spd(Sig{b}, Sig_inv{b}, gradLB_Sig_momentum{b}, stepsize);
                end

                for b = 1:setting.Block.n
                    % Sig_inv needed in retraction, parallel transport and
                    % h_function
                    [L{b},Sig_inv{b}] = mk_L_Sig(Sig{b});
                end
                ...
                for b = 1:n_blks
                    zeta{b} = obj.parallel_transport_spd(Sig_inv_old{b}, Sig{b}, gradLB_Sig_momentum{b});
                    gradLB_Sig_momentum{b} = momentum_weight*zeta{b}+(1-momentum_weight)*gradLB_Sig{b};
                end
